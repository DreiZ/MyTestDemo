...
@interface ZOrganizationCardSendVC ()
@property (nonatomic,strong) UIButton *bottomBtn;
@property (nonatomic,strong) UIButton *navLeftBtn;

@end

@implementation ZOrganizationCardSendVC
...
- (void)initCellConfigArr {
    ...
    NSArray *textArr = @[@[@"类型", @"请选择可用课程", @NO, @"rightBlackArrowN", @"type",@30, @"",ftitle,[NSNumber numberWithInt:ZFormatterTypeAny]],
                         ...
                         @[@"指定学员", @"请选择学员", @NO, @"rightBlackArrowN", @"student",@40, @"",ValidArray(self.viewModel.addModel.student) ? @"已选择":@"",[NSNumber numberWithInt:ZFormatterTypeAny]],
                         @[@"每人赠送", @"0", @YES, @"", @"preNum",@3, @"张",SafeStr(self.viewModel.addModel.limit),[NSNumber numberWithInt:ZFormatterTypeNumber]]];
...
- (void)setNavigation {
    [self.navigationItem setTitle:@"赠送卡券"];
...
- (UIButton *)bottomBtn {
    if (!_bottomBtn) {
        ...
        [_bottomBtn setTitle:@"赠送优惠券" forState:UIControlStateNormal];
        ...
        [_bottomBtn bk_addEventHandler:^(id sender) {
            ...
            if (!ValidArray(weakSelf.viewModel.addModel.student)) {
            ...
            if ([weakSelf.viewModel.addModel.limit intValue] == 0) {
                [TLUIUtility showErrorHint:@"赠送数量不能为0"];
                return ;
            }
...
//            }
            ...
            for (int i = 0; i < weakSelf.viewModel.addModel.student.count; i++) {
                ZOriganizationStudentListModel *model = weakSelf.viewModel.addModel.student[i];
                [students addObject:model.code_id];
            }
            [params setObject:students forKey:@"student"];
...
- (void)updateDataWithParams:(NSMutableDictionary *)otherDict {
    ...
    __weak typeof(self) weakSelf = self;
    [ZOriganizationCardViewModel addSengCart:otherDict completeBlock:^(BOOL isSuccess, NSString *message) {
...
-(void)zz_tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath cellConfig:(ZCellConfig *)cellConfig {
    ...
    if ([cellConfig.title isEqualToString:@"type"]) {
        ...
        avc.handleBlock = ^(NSArray * ids) {
            [weakSelf.viewModel.addModel.student removeAllObjects];
            [weakSelf.viewModel.addModel.student addObjectsFromArray:ids];
            [weakSelf initCellConfigArr];
            [weakSelf.iTableView reloadData];
        };
