...
//            }
            ...
            for (int i = 0; i < self.viewModel.addModel.student.count; i++) {
                ZOriganizationStudentListModel *model = self.viewModel.addModel.student[i];
...
-(void)zz_tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath cellConfig:(ZCellConfig *)cellConfig {
    ...
    if ([cellConfig.title isEqualToString:@"type"]) {
        ...
        avc.handleBlock = ^(NSArray * ids) {
            [self.viewModel.addModel.student removeAllObjects];
            [self.viewModel.addModel.student addObjectsFromArray:ids];
            [self initCellConfigArr];
            [self.iTableView reloadData];
        };
