//
//  ZOrganizationAccountSchoolListLogsCell.m
//  NoWait
//
...
@interface ZOrganizationAccountSchoolListLogsCell ()
...
@implementation ZOrganizationAccountSchoolListLogsCell
...
- (void)setModel:(ZStoresAccountDetaliListLogModel *)model {
    _model = model;
    _daoLabel.text = [NSString stringWithFormat:@"￥%@",SafeStr(model.money)];
    _feiLabel.text = @"";
    _timeLabel.text = [NSString stringWithFormat:@"%@",[SafeStr(model.updated_at) timeStringWithFormatter:@"yyyy-MM-dd HH:MM"]];
    _zhiLabel.text = [NSString stringWithFormat:@"￥%@",SafeStr(model.person)];
    _zhuanPriceLabel.text = @"";
//    [NSString stringWithFormat:@"￥%@",SafeStr(model.should_amount)];
}
