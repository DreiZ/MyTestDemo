...
@implementation ZAlertStoreInfoView

static ZAlertStoreInfoView *sharedManager;

+ (ZAlertStoreInfoView *)sharedManager {
    @synchronized (self) {
        if (!sharedManager) {
            sharedManager = [[ZAlertStoreInfoView alloc] init];
        }
    }
...
- (void)setName:(NSString *)title data:(NSString *)data {
...
+ (void)setAlertName:(NSString *)title data:(NSString *)data  {
    [[ZAlertStoreInfoView sharedManager] setName:title data:data];
}
