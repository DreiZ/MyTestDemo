//
//  ZOrganizationTextLabelListCell.m
//  NoWait
//
...
#define btnHeight CGFloatIn750(78)
#define btnAddWidth CGFloatIn750(40)
#define leftX CGFloatIn750(20)

@interface ZOrganizationTextLabelListCell ()<UITextFieldDelegate>
...
@implementation ZOrganizationTextLabelListCell
...
+ (CGFloat)z_getCellHeight:(id)sender {
    ...
    if (model.data && [model.data isKindOfClass:[NSArray class]]) {
        ...
        if (_hotProductArr && _hotProductArr.count > 0) {
            offSetY = CGFloatIn750(17) + _hotProductArr.count * btnHeight + (_hotProductArr.count - 1) * CGFloatIn750(20);
        }else{
            return CGFloatIn750(86);
        }
        
        return offSetY + CGFloatIn750(17);
    }else {
        return CGFloatIn750(86);
    }
}
...
- (void)setHotProductArr:(NSArray *)hotProductArr {
    ...
    for (int i = 0; i < _hotProductArr.count; i++) {
        ...
        NSDictionary *tempDict = _hotProductArr[i];
        ...
        [nameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(btn.mas_right).offset(-CGFloatIn750(20));
            make.left.equalTo(btn.mas_left).offset(CGFloatIn750(20));
            make.bottom.equalTo(btn.mas_centerY).offset(-CGFloatIn750(4));
        }];

        UILabel *priceLabel = [[UILabel alloc] initWithFrame:CGRectZero];
        priceLabel.textColor = [UIColor colorMain];
        if ([tempDict objectForKey:@"price"]) {
            priceLabel.text = [NSString stringWithFormat:@"标准价:%@",tempDict[@"price"]];
        }
        ...
        [priceLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(btn.mas_left).offset(CGFloatIn750(20));
            make.top.equalTo(btn.mas_centerY).offset(CGFloatIn750(1));
        }];

        UILabel *spriceLabel = [[UILabel alloc] initWithFrame:CGRectZero];
        spriceLabel.textColor = [UIColor colorMain];
        if ([tempDict objectForKey:@"teacherPrice"]) {
            spriceLabel.text = [NSString stringWithFormat:@"带课价:%@",tempDict[@"teacherPrice"]];
        }

        spriceLabel.textAlignment = NSTextAlignmentLeft;
        [spriceLabel setFont:[UIFont fontSmall]];
        [btn addSubview:spriceLabel];
        [spriceLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(priceLabel.mas_right).offset(CGFloatIn750(20));
            make.top.equalTo(btn.mas_centerY).offset(CGFloatIn750(1));
        }];

        [self.labelView addSubview:btn];

        btn.frame = CGRectMake(offSetX, offSetY, maxWidth-CGFloatIn750(40), btnHeight);
