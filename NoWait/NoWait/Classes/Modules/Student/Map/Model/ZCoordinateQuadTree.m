...
@implementation ZCoordinateQuadTree
...
- (NSArray *)getAnnotationsWithoutClusteredInMapRect:(MAMapRect)rect {
    ...
    QuadTreeGatherDataInRange(self.root, BoundingBoxForMapRect(rect), ^(QuadTreeNodeData data) {
        ...
        ZClusterAnnotation *annotation = [[ZClusterAnnotation alloc] initWithCoordinate:coordinate count:1];
    ...
    for (NSInteger x = minX; x <= maxX; x++)
    {
        for (NSInteger y = minY; y <= maxY; y++)
        {
            ...
            if (count == 1)
            {
                ...
                ZClusterAnnotation *annotation = [[ZClusterAnnotation alloc] initWithCoordinate:coordinate count:count];
...
- (NSArray<ZClusterAnnotation *> *)clusteredAnnotationsWithinMapRect:(MAMapRect)rect withDistance:(double)distance {
    ...
    NSMutableArray<ZClusterAnnotation *> *clusteredAnnotations = [[NSMutableArray alloc] init];
    for (AMapPOI *aAnnotation in allAnnotations) {
        ...
        ZClusterAnnotation *cluster = [self getClusterForAnnotation:aAnnotation inClusteredAnnotations:clusteredAnnotations withDistance:distance];
        if (cluster == nil) {
            ZClusterAnnotation *aResult = [[ZClusterAnnotation alloc] initWithCoordinate:resultCoor count:1];
...
- (ZClusterAnnotation *)getClusterForAnnotation:(AMapPOI *)annotation inClusteredAnnotations:(NSArray<ZClusterAnnotation *> *)clusteredAnnotations withDistance:(double)distance {
    ...
    for (ZClusterAnnotation *aCluster in clusteredAnnotations) {
