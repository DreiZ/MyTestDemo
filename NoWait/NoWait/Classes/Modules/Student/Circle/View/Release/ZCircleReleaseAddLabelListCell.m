//
//  ZCircleReleaseAddLabelListCell.m
//  NoWait
//
...
@interface ZCircleReleaseAddLabelListCell ()
@property (nonatomic,strong) UIView *labelView;
@property (nonatomic,strong) NSMutableArray *btnArr;

@end

@implementation ZCircleReleaseAddLabelListCell
...
- (void)setLabel {
    ...
    for (int i = 0; i< _list.count; i++) {
        ...
        CGFloat width = [self getTheStringWidth:[NSString stringWithFormat:@"%@%@", title,labelBack] font:[UIFont systemFontOfSize:14]];
        width += btnAddWidth;
...
-(UIButton*) getHotSearchBtnItem:(NSInteger)index
{
    if (index >= _btnArr.count) {
        ...
        [btn setTitleColor:[UIColor colorWhite] forState:UIControlStateNormal];
        btn.layer.cornerRadius = btnHeight/2.0;
        btn.layer.masksToBounds = YES;
        btn.backgroundColor = adaptAndDarkColor([UIColor colorMain], [UIColor colorMain]);
