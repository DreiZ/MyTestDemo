//
//  ZStudentMineOrderDetailResultCell.m
//  NoWait
//
...
@interface ZStudentMineOrderDetailResultCell ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation ZStudentMineOrderDetailResultCell
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([cellConfig.title isEqualToString:@"seeOrder"]){
        ZStudentMineSettingBottomCell *lcell = (ZStudentMineSettingBottomCell *)cell;
        lcell.titleLabel.textColor = [UIColor colorMain];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cellConfig.title isEqualToString:@"seeOrder"]) {
        if (_handleBlock) {
            if (self.handleBlock) {
                self.handleBlock(ZLessonOrderHandleTypeDetail, self.model);
            }
        }
    }
}
...
//                [self setOrderDetailMidViewBottom];
    ...
    [self.cellConfigArr addObject:getGrayEmptyCellWithHeight(0.5)];
    ZCellConfig *switchCellConfig = [ZCellConfig cellConfigWithClassName:[ZStudentMineSettingBottomCell className] title:@"seeOrder" showInfoMethod:@selector(setTitle:) heightOfCell:CGFloatIn750(106) cellType:ZCellTypeClass dataModel:@"查看订单"];
...
- (void)setDetailSubViewBottom {
    ...
    [self.subView mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.height.mas_equalTo(CGFloatIn750(56*3 + 28)+CGFloatIn750(106));
    }];
}
    ...
    if (sender && [sender isKindOfClass:[ZOrderDetailModel class]]) {
        ...
        if (!listModel.isRefund && [listModel.has_can_refund intValue] == 1) {
            if ([[ZUserHelper sharedHelper].user.type intValue] == 1) {
                return CGFloatIn750(328 + 56 * 3 + 28 + 56)+CGFloatIn750(86);
            }else{
                return CGFloatIn750(328 + 56 * 3 + 28 )+CGFloatIn750(86);
            }
        ...
        switch (listModel.order_type) {
            case ZOrganizationOrderTypeForPay:
            case ZStudentOrderTypeForPay://待付款（去支付，取消）
                return CGFloatIn750(328 + 56 * 3 + 28)+CGFloatIn750(56);
                break;
            case ZOrganizationOrderTypeHadPay:
                return CGFloatIn750(328 + 56 * 3 + 28)+CGFloatIn750(56);
                break;
            ...
            case ZOrganizationOrderTypeHadEva:
                ...
                break;
            case ZStudentOrderTypeHadEva://完成已评价(删除)
                return CGFloatIn750(328 + 56 * 3 + 28 + 56)+CGFloatIn750(56);
                break;
            case ZOrganizationOrderTypeOrderOutTime:
            case ZOrganizationOrderTypeOutTime:
                return CGFloatIn750(328 + 56 * 3 + 28)+CGFloatIn750(56);
            ...
            case ZOrganizationOrderTypeOrderRefuse:
            case ZStudentOrderTypeOrderRefuse://已拒绝（预约）
                    return CGFloatIn750(328)+CGFloatIn750(56);
                break;
            default:
                break;
        }
