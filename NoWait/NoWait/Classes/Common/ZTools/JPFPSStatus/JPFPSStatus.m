...
- (id)init {
    ...
    if (self) {
        ...
        // fpsLabel
        fpsLabel = [[UILabel alloc] initWithFrame:CGRectMake(KScreenWidth - 150, safeAreaTop(), 50, 20)];
...
- (void)open {
    ...
    for (UIView *label in rootVCViewSubViews) {
        if ([label isKindOfClass:[UILabel class]]&& label.tag==10001) {
            return;
        }
    }

    [displayLink setPaused:NO];
    [[AppDelegate shareAppDelegate].window addSubview:fpsLabel];
}
