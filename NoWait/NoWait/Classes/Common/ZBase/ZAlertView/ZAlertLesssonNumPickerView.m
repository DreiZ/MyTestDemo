...
@interface ZAlertLesssonNumPickerView ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
@implementation ZAlertLesssonNumPickerView

static ZAlertLesssonNumPickerView *sharedManager;

+ (ZAlertLesssonNumPickerView *)sharedManager {
    @synchronized (self) {
        if (!sharedManager) {
            sharedManager = [[ZAlertLesssonNumPickerView alloc] init];
        }
    }
...
- (void)setName:(NSString *)title items:(NSInteger)data handlerBlock:(void(^)(NSInteger))handleBlock {
...
- (void)setName:(NSString *)title selectedIndex:(NSInteger)index items:(NSInteger)data handlerBlock:(void(^)(NSInteger))handleBlock {
    self.handleBlock = handleBlock;
    self.nameLabel.text = title;
    self.data1 = data;
    self.proIndex = index;
...
+ (void)setAlertName:(NSString *)title items:(NSInteger)data handlerBlock:(void(^)(NSInteger))handleBlock  {
    [[ZAlertLesssonNumPickerView sharedManager] setName:title items:data handlerBlock:handleBlock];
}
...
+ (void)setAlertName:(NSString *)title selectedIndex:(NSInteger)index items:(NSInteger)data handlerBlock:(void(^)(NSInteger))handleBlock  {
    [[ZAlertLesssonNumPickerView sharedManager] setName:title selectedIndex:index items:data handlerBlock:handleBlock];
}
...
-(NSString*)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component{
    if (component == 0) {
        if (row < self.data1) {
            return [NSString stringWithFormat:@"第%ld节",row + 1];
        }
