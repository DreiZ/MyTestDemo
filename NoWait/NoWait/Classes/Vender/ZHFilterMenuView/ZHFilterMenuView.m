...
@property (nonatomic, strong) UIView *fileterBackView;
...
//        [self.superview bringSubviewToFront:self];
        [self.fileterBackView addSubview:self.backGroundView];
        if (downType == ZHFilterMenuDownTypeTwoLists) {
            self.leftTableView.frame = CGRectMake(0, 0, KScreenWidth / 3, 0);
            self.rightTableView.frame = CGRectMake(KScreenWidth / 3, 0, self.frame.size.width / 3 * 2, 0);
            [self.fileterBackView addSubview:self.leftTableView];
            [self.fileterBackView addSubview:self.rightTableView];
        } else {
            self.leftTableView.frame = CGRectMake(0, 0, KScreenWidth, 0);
            [self.fileterBackView addSubview:self.leftTableView];
            [self.fileterBackView removeFromSuperview];
        }
        ...
        if (confirmType == ZHFilterMenuConfirmTypeBottomConfirm) {
            ...
            [self.fileterBackView addSubview:self.bottomView];
            bottomHeight = self.bottomHeight;
        }
        viewHeight = MAX(viewHeight, self.maxHeight - bottomHeight);
        ...
        [MenuWindow addSubview:self.fileterBackView];
    } else {
        [UIView animateWithDuration:0.3 animations:^{
...
- (UIView *)fileterBackView {
    if (!_fileterBackView) {
        _fileterBackView = [[UIView alloc] initWithFrame:CGRectMake(0, kTopHeight+CGFloatIn750(88), KScreenWidth, KScreenHeight-kTopHeight-CGFloatIn750(88))];
        _fileterBackView.layer.masksToBounds = YES;
    }
    return _fileterBackView;
}
/*
