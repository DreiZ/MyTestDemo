...
@property (nonatomic, assign) CGFloat animationScaleX;//缩放比例
@property (nonatomic, assign) CGFloat animationScaleY;//缩放比例
@end
...
- (void)pushAnimateTransition:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    //计算缩放比例
    _animationScaleX = MAX([toVC targetTransitionView].width, snapShot.width) / MIN([toVC targetTransitionView].width, snapShot.width);
    _animationScaleY = MAX([toVC targetTransitionView].height, snapShot.height) / MIN([toVC targetTransitionView].height, snapShot.height);
    
    CGRect originFrame = fromView.frame;
    [UIView animateWithDuration:self.animationDuration animations:^{
        //设置缩放变换 x,y分别放大多少倍
        snapShot.transform =  CGAffineTransformMakeScale(_animationScaleX,_animationScaleY);
        [snapShot setOrigin:toViewPoint];
        
        fromView.alpha = 0;
        fromView.transform = snapShot.transform;
        
        CGPoint snowViewPoint = [fromCellView convertPoint:CGPointZero toView:nil];
        CGFloat offsetY = 0;
        CGFloat offsetX = 0;
        if (snowViewPoint.x > toViewPoint.x) {
            offsetX += - (snowViewPoint.x - toViewPoint.x);
        }else {
            offsetX += toViewPoint.x - snowViewPoint.x;
        }
        
        if (snowViewPoint.y > toViewPoint.y) {
            offsetY += - (snowViewPoint.y - toViewPoint.y);
        }else {
            offsetY += toViewPoint.y - snowViewPoint.y ;
        }

        
        fromView.frame = CGRectMake(offsetX + fromVC.view.frame.origin.x,
                                    offsetY + fromVC.view.frame.origin.y,
                                    fromVC.view.frame.size.width,
                                    fromVC.view.frame.size.height);
        
//        [ZLaunchManager sharedInstance].tabBarController.tabBar.alpha = 0;
...
- (void)popAnimateTransition:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    //获取相对窗口的坐标
    CGPoint leftUperPoint = [toCellView convertPoint:CGPointZero toView:toVC.view];
...
//    CGFloat offsetY = fromVC.navigationController.navigationBarHidden ? 0.0 : 64;
    ...
    //计算缩放比例
    _animationScaleX = MAX(fromCellView.width, snapShot.width) / MIN(fromCellView.width, snapShot.width);
    _animationScaleY = MAX(fromCellView.height, snapShot.height) / MIN(fromCellView.height, snapShot.height);
                                                                    
    [containerView addSubview:snapShot];
    snapShot.backgroundColor = [UIColor clearColor];
    //将目标View先放大到跟当前view一样大，然后在动画中缩小，实现动画pop效果
    snapShot.transform = CGAffineTransformMakeScale(_animationScaleX, _animationScaleY);
    [snapShot setOrigin:CGPointMake(nowViewPoint.x, nowViewPoint.y)];
    //用于动画设置淡出缩小效果
    ...
    toView.frame = CGRectMake(-(leftUperPoint.x * _animationScaleX), -((leftUperPoint.y - nowViewPoint.y) * _animationScaleY + nowViewPoint.y),
...
//        [ZLaunchManager sharedInstance].tabBarController.tabBar.alpha = 1;
    } completion:^(BOOL finished) {
        //        if (finished) {
