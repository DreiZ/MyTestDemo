...
-(void)updateStarImageView:(UIView *)view
{
    NSArray *imageViewArr = view.subviews;
    [imageViewArr enumerateObjectsUsingBlock:^(UIView  *imageView, NSUInteger idx, BOOL * _Nonnull stop) {
        imageView.frame = CGRectMake(imageView.tag * self.bounds.size.width / self.numberOfStars, 0, self.bounds.size.width / self.numberOfStars, self.bounds.size.height);
    }];
}
...
- (void)layoutSubviews {
    ...
    [self updateStarImageView:_foregroundStarView];
    [self updateStarImageView:_backgroundStarView];
